...
//+------------------------------------------------------------------+
	...
	function pressbutton(element){
      ...
      console.time('Test performance');  //Inicia la medicion de velocidad de la instrucion.
	  
	  var name = element.getAttribute("name");
	  var isNumber = isNaN(name); //Verifica si el caracter es numerico.
	  		  
	    if(isNumber == false || name == 'raiz' || name == 'raiz_1' || name == '1/' || name == 'pi' || name == 'sin' || name == 'cos' || name == 'tan' || name == 'asin' || 
	       name == 'acos' || name == 'atan' || name == '10^' || name == 'e^' || name == 'log' || name == 'ln' || name == 'n!' || name == '^2' || name == 'raizxy' || name == '+' || 
	       name == '-'|| name == '*' || name == '/' || name == '.' || name == '%from' || name == 'Mod' || name == '^' || name == 'Deg' || name == 'F-E' || name == 'signo' || 
	       name == '(' || name == ')' || name == 'delete' || name == 'sum' || name == 'C' || name == 'CE' || name == 'M')
	    {
...
//+------------------------------------------------------------------+
	...
	function getCursorPositionInScreen(element){
	   ...
	   if(screen.innerHTML.length > 1){
         ...
         var actualPosition = document.getSelection().getRangeAt(0).startOffset;
		 ...
		 for(h=0, len = arrayOperatorsAndFunctions.length; h< len; h++)
		 {
		  if(arrayScreenContainer[h].charAt(0) == "|" || len-1 == h)
		  {
			arrayScreenContainer[h] = converterCharacter(arrayOperatorsAndFunctions[h]);
			lastCursorPosition = h;
            break;			
		  }
		 }
	     ...
	     if(actualPosition == "0"){
		  ...
		  //El cursor nunca puede ocupar la posicion "0".
		    arrayScreenContainer[1] = "|" + arrayScreenContainer[1];
	     }
		 
	    else{
		    if(actualPosition > lastCursorPosition && actualPosition <= len-1){
	          arrayScreenContainer[actualPosition - 1] = "|" + arrayScreenContainer[actualPosition - 1];
	        }
			
		  else if(actualPosition > lastCursorPosition && actualPosition > len-1){
	          arrayScreenContainer[len-1] = arrayScreenContainer[len-1] + "|";
	        }
			else{
			  arrayScreenContainer[actualPosition] = "|" + arrayScreenContainer[actualPosition];	
			}
	    }
	  }
	   ...
	   {
		   if(arrayScreenContainer[i].charAt(0) != "|")
		   {
	         expressionToCalculate += arrayScreenContainer[i];
		   }
	     else{
			 var partOfExpression = arrayScreenContainer[i].substring(1);
			 expressionToCalculate += "|" + partOfExpression;
		 }
	   }
	   screen.innerHTML = expressionToCalculate;
       document.activeElement.blur(); //Oculta el teclado de android.	   
	}
...
//+------------------------------------------------------------------+
	...
	function editingText(element){
	  ...
	  if(name == 'left')
	  {
	    for(h=0, len = arrayOperatorsAndFunctions.length; h<len; h++)
		{
		  if(arrayScreenContainer[h].charAt(0) == "|" && h != 1)
		  {
		    arrayScreenContainer[h] = converterCharacter(arrayOperatorsAndFunctions[h]);
		    arrayScreenContainer[h-1] = "|" + arrayScreenContainer[h-1];
			goLeft = true;
			break;
		  }
		else if(arrayScreenContainer[h].charAt(0) == "|" && h == 1)
		  {
		    arrayScreenContainer[h] = converterCharacter(arrayOperatorsAndFunctions[h]);
		    arrayScreenContainer[len-1] = arrayScreenContainer[len-1] + "|";
			goLeft = true;
		  ...
		  if(goLeft == false && len-1 == h){
		    
			arrayScreenContainer[1] = converterCharacter(arrayOperatorsAndFunctions[1]);
			arrayScreenContainer[h] = converterCharacter(arrayOperatorsAndFunctions[h]);
		    arrayScreenContainer[h] = "|" + arrayScreenContainer[h];
		  }
        }		
	  }
	else if(name == 'right')
	  {
		...
		{
		  if(arrayScreenContainer[i].charAt(0) == "|" && i != len-1)
		  {
		    arrayScreenContainer[i] = converterCharacter(arrayOperatorsAndFunctions[i]);
		    arrayScreenContainer[i+1] = "|" + arrayScreenContainer[i+1];
			goRight = true;
			break;
		  }
		  
		  if(goRight == false && arrayScreenContainer[i].charAt(0) == "|" && len-1 == i){
		    
			arrayScreenContainer[i] = converterCharacter(arrayOperatorsAndFunctions[i]);
		    arrayScreenContainer[i] = arrayScreenContainer[i] + "|";
		  }	
        } 
	  }
...
//+------------------------------------------------------------------+
    ...
    function verifyAddOfShapesInPosition(){
        
		var h, len;
		
	    for(h=0, len = arrayOperatorsAndFunctions.length; h<len; h++)
		{
		  if(arrayScreenContainer[h].charAt(26) == "|")
		  {
		    return true;
		  }
	    }	
	 ...
	 var expressionToCalculate = "";
	    ...
	    if(characterNumeric == false)
        {
		   if(statusCharactersNumeric == true){
		      ...
		      var verifyParenthesisClose = arrayOperatorsAndFunctions[arrayOperatorsAndFunctions.length - 1]; //Verifica si el ultimo caracter del array es un parentesis de cierre.

		       if(verifyParenthesisClose != ')' && verifyParenthesisClose != 'pi')
	           { 
		...
		{ 
		   ...
		   if(name == "^2"){
		     ...
		     if(calculatingWithTheResult == false){
		           ...
		           if(partialDetectionGlobal.isNumber == false && partialDetectionGlobal.operatorNameDetected == true && verifyAddOfShapesInPosition() == false ||
     			      partialDetectionGlobal.isNumber == false && partialDetectionGlobal.otherOperatorNameDetected == false && verifyAddOfShapesInPosition() == false)
	               {
		             ...
		             {
			           arrayScreenContainer.push(potentiationOperatorsSquared[i]); //El array almacena el operador y el numero exponente de la potencia, en este caso, el exponente numero "2".
			           arrayOperatorsAndFunctions.push(potentiationOperatorsSquared[i]);
			         }
			          statusCharactersNumeric = false;
			          errorDetector(element,true); //Cambia el color del boton a "azul", si es correcta la sintaxis.
		           }
		         else if(verifyAddOfShapesInPosition() == false) //Verifica la posicion del cursor en el texto.
			       {
				      errorDetector(element,false); //Cambia el color del boton a "rojo", si es incorrecta la sintaxis. 
			       }
		         else if(verifyAddOfShapesInPosition() == true) //Verifica la posicion del cursor en el texto.
			       { 
			           var arrayTemporaryOperatorsAndFunctions = new Array(); //Se declaran array temporales.
			           var arrayTemporaryScreenContainer = new Array();
			           var p = 0;
			   
			           for(k=0, len = potentiationOperatorsSquared[i]; k<len; k++)//Almacena en los array temporales hasta el simbolo "|".
		               {
			             arrayTemporaryOperatorsAndFunctions.push(arrayOperatorsAndFunctions[k]);
				         arrayTemporaryScreenContainer.push(arrayScreenContainer[k]);
				 
		                 if(arrayScreenContainer[k+1].charAt(26) == "|")
		                 {
		                   p = k+1;
				           break;
				         }
			           ...
			           var partialDetection = PartialVerificationOfOperatorsAndFunctions(arrayTemporaryOperatorsAndFunctions,name); //Aqui verifica si la expresion cumple los requerimientos minimos para poder adicionar los caracteres deseados.

			           if(partialDetection.isNumber == false && partialDetection.operatorNameDetected == true || partialDetection.isNumber == false && 
			              partialDetection.otherOperatorNameDetected == false)
			           {
			             for(i=0, len = potentiationOperatorsSquared.length; i<len; i++)//Se adicionan los elementos deseados al array.
		                 {
			               arrayTemporaryScreenContainer.push(potentiationOperatorsSquared[i]); //El array almacena el operador y el numero exponente de la potencia, en este caso, el exponente numero "2".
			               arrayTemporaryOperatorsAndFunctions.push(potentiationOperatorsSquared[i]);
			             }
				 
				         for(k=p, len = arrayOperatorsAndFunctions.length; k<len; k++)
		                 {
		                   arrayTemporaryOperatorsAndFunctions.push(arrayOperatorsAndFunctions[k]);//Se terminan de adicionar los elementos restantes al array temporal.
				           arrayTemporaryScreenContainer.push(arrayScreenContainer[k]);
	                     }
				 
				         arrayOperatorsAndFunctions.splice(0,arrayOperatorsAndFunctions.length-1);//se limpian los array originales.
				         arrayScreenContainer.splice(0,arrayScreenContainer.length-1);
				         arrayOperatorsAndFunctions = arrayTemporaryOperatorsAndFunctions;//Aqui los array originales reciben los elementos de los array temporales.
				         arrayScreenContainer = arrayTemporaryScreenContainer;
      ...
      else if(name == "signo")
        {
		    ...
		    if(calculatingWithTheResult == false){
		      ...
		      { 
                if(arrayOperatorsAndFunctions[arrayOperatorsAndFunctions.length - 1] == nameOfOperators[i]) ////Verifica si el ultimo caracter del array es un operador.
		        {
		          operatorNameDetected = nameOfOperators[i];
			      break;
		        }
		      }
		  
		   	 
		       if(arrayOperatorsAndFunctions[arrayOperatorsAndFunctions.length - 1] == 'signo') //Verifica el estado actual del operador especial.
		       {
		         arrayOperatorsAndFunctions.splice(arrayOperatorsAndFunctions.length - 1,1); //Borra el ultimo caracter del array.
			     arrayScreenContainer.splice(arrayScreenContainer.length - 1,1);; //Borra el ultimo caracter del array.
		       }
               ...
               {
		          if(isNumber == false || operatorNameDetected == "pi" || operatorNameDetected == "(")
	              {
		            screen.innerHTML += "-";
			        arrayOperatorsAndFunctions.push(name);
			        arrayScreenContainer.push("-");
			      }
		        else if(arrayOperatorsAndFunctions.length == 0)
	              {
		            screen.innerHTML += "-";
			        arrayOperatorsAndFunctions.push(name);
			        arrayScreenContainer.push("-");
	    ...
	    {
		    var nameOfOperators = ['(','+','-','*','/','sin','cos','tan','asin','acos','atan','raiz','raiz_1','raizxy','log','ln','n!','signo']; //Nombre de operadores y funciones.
		    var operatorNameDetected = false;
		...
		{
		    memoryResult(element);
		}
		
         console.timeEnd('Test performance'); //Finaliza la medicion de velocidad de la instrucion.
 }
...
//+------------------------------------------------------------------+  
	function PartialVerificationOfOperatorsAndFunctions(array,name){
          ...
          var nameOfOperators = ['+','-','*','/','(','raizxy','.']; //Nombre de operadores de botones.								   
		  var operatorNameDetected = false;
...
//+------------------------------------------------------------------+	
	...
	function repetitiveExecutionProcedureOperatorsPreceding(name){ //Funcion que ejecuta procedimientos repetitivos dentro de la **FUNCION:verifyOperators**.
		screen.innerHTML += converterCharacter(name); //La caja de texto "screen" recibe los caracteres numericos ó operadores, despues de pasar por la funcion "converterCharacter".
	    arrayScreenContainer.push(converterCharacter(name)); //El array almacena la expresion igual como aparece en pantalla, en la caja de texto "screen".
	    arrayOperatorsAndFunctions.push(name);
	}
...
//+------------------------------------------------------------------+
	...
	function resetCalculator(name,element){ //Funcion que reinicia la calculadora con el boton "C".
		
		screen.innerHTML = ""; //Limpia la caja de texto.
		screen1.innerHTML = ""; //Limpia la caja de texto.
		screen11.innerHTML = ""; //Limpia la caja de texto.
		document.getElementById("memory_1").style.backgroundColor = "#3d3d3d"; //Cambia el color de fondo del boton "M".
		document.getElementById("memory").style.backgroundColor = "#3d3d3d"; //Cambia el color de fondo del boton "M".
		arrayOperatorsAndFunctions.splice(0,arrayOperatorsAndFunctions.length); //Limpia el array.
		arrayScreenContainer.splice(0,arrayScreenContainer.length); //Limpia el array.
		arrayGuardingPlaceFromAParenthesis.splice(0,arrayGuardingPlaceFromAParenthesis.length); //Limpia el array.
        statusMemory = false;
...
//+------------------------------------------------------------------+
	...
	function memoryResult(element){ //Almacena en memoria el resultado de la expresion matematica.
	  ...
	  else if(statusMemoryResult == true && statusMemory == false)
		{
		  ...
		  { 
           	if(arrayOperatorsAndFunctions[arrayOperatorsAndFunctions.length - 1] == operatorsBasic[i])
			{
			  ...
			  element.style.backgroundColor = "#3d3d3d"; //Cambia el color de fondo del boton con id "memory y memory_01".
			  screen11.innerHTML = "";
			  statusMemoryResult = false;
			  statusNumericalresultsInScreen = true;
			  break;
...
//+------------------------------------------------------------------+
	...
	function sum(name,element){ //Funcion que resuelve la expresion matematica con la funcion "eval".
	   ...
	   var operatorsAndFunctions = ['+','-','*','/','%from','raiz','raiz_1','raizxy','signo','1/','^','(','sin','cos','tan','asin','acos','atan','log','ln','n!','Deg','F-E','Mod'];
	   var statusSum = true;
...
//+------------------------------------------------------------------+
    ...
    function resolveOperations(array,numberx,numbery,Potentiation,Percentage,RootSquare,rootSquareTotal,Reciproc,statusTrigonometricIdentities,residueDivision,rootxy,arrayRootxy)
	{
		 ...
		 {
		   var porcentaje = (numbery*numberx)/100;
		   array[array.length] = porcentaje;
		 }
		 
	   else if(RootSquare == true) //Calcula la raiz cuadrada.
		 {
		   var root = Math.sqrt(numbery);

		   if(rootSquareTotal>1)
		   {
	   ...
	   else if(Reciproc == true) //Obtiene el resultado de la division del numero 1 entre cualquier otro numero.
		 {
		   var reciproc = 1/numbery;
		   array[array.length] = reciproc;
		}
	...
	function calculatePartial(array){ //Realiza calculos parciales dentro de la expresion a resolver.
		...
		var arrayResult = new Array();
	     ...
	     for(var i=0; i<array.length; i++)
		 {
			...
			else if (array[i] == "1/") //Obtiene el resultado de la division del numero 1 entre cualquier otro numero.
			  {
			    arrayResult = verifyArrayResultOfOperatorsAndNumbers(arrayResult); //Verifica y corrige los elementos del array.
			    statusNumber = false;
		        statusReciproc = true;
		  ...
		  //Funcion que realiza las operaciones matematicas.	
	        arrayResult = resolveOperations(arrayResult,numberx,numbery,statusPotentiation,statusPercentage,statusRootSquare,rootSquareTotal,statusReciproc,
			                                statusTrigonometricIdentities,statusResidueDivision,statusRootxy,arrayRootxyTotal);																												

				    if (array[i] == "signo") //A partir de aqui se adicionan los operadores.
			        {
